...
//	private static MTRandom _rnd = new MTRandom();
	private static Random _rnd = ThreadLocalRandom.current();
	...
	{
		return  Math.floor(_rnd.nextDouble() * n);
	}

	public static double get(double min, double max) // get random number from min to max (not max-1 !)
	{
	...
	public static int get(int min, int max, int[] list) // get random number from min to max (not max-1 !)
	{
		int rezult;
		rezult = min + (int) Math.floor(_rnd.nextDouble() * (max - min + 1));
		for(int id : list)
			if(rezult == id)
				get(min, max, list);
		return rezult;
