...
public class Strings
{
	...
	public static Integer parseInt(Object x)
	{
		...
		try
		{
			res = Integer.parseInt(String.valueOf(x));
		}
		catch(Exception e)
		{}
		return res;
	...
	public static Double parseFloat(Object x)
	{
		...
		try
		{
			res = Double.parseDouble(String.valueOf(x));
		}
		catch(Exception e)
		{}
		return res;
...
//		s = replace(s, "\\[([^\\]\\|]*?)\\|([^\\]]*?)\\]", Pattern.DOTALL, "<a action=\"bypass -h $1\">$2</a>");
		s = replace(s, "\\[([^\\]\\|]*?)\\|([^\\]]*?)\\]", Pattern.DOTALL, "<button action=\"bypass -h $1\" value=\"$2\" width=320 height=32 back=\"ReLifeBTN.Btn_Back512\" fore=\"ReLifeBTN.Btn_Font512\">");
		s = s.replaceAll(" @", "\" msg=\"");
		s = s.replace("\"\"", "\"");
