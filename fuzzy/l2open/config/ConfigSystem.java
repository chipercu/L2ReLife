...
public class ConfigSystem
{
	...
	private static void ipsLoad()
	{
		...
		try
		{
			...
			InputStream is = new FileInputStream(new File("./config/advipsystem.properties"));
		...
		{
			e.printStackTrace();
			throw new Error("Failed to Load ./config/advipsystem.properties File.");
		}
	}
	...
	public static void loadGMAccess(File file)
	{
		...
		{
			e.printStackTrace();
			throw new Error("Failed to Load ./config/GMAccess.xml File. [" + file.getPath() + "]");
		}
	}
	...
	{
		saveHexid(string, "./config/hexid.txt");
	}
