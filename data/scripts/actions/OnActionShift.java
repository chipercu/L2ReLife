...
public class OnActionShift extends Functions implements ScriptFile {
    ...
    public boolean OnActionShift_L2NpcInstance(L2Player player, L2Object object) {
        ...
        if (object.isNpc()) {
            ...
            if (player.isGM())
                dialog = dialog.replaceFirst("%AI%", String.valueOf(npc.getAI()) + ",<br1>active: " + npc.getAI().isActive() + ",<br1>intention: " + npc.getAI().getIntention() + ",<br1>target: " + npc.getTarget());
            else
    ...
    public void moveW() {
        ...
        Location loc = new Location(npc.getX(), npc.getY() + 10, npc.getZ(), npc.getHeading());
        npc.teleToLocation(loc, player.getReflectionId());
        location();
    }
    ...
    public void moveA() {
        L2Player player = (L2Player) getSelf();
        L2NpcInstance npc = (L2NpcInstance) player.getTarget();
        Location loc = new Location(npc.getX() + 10, npc.getY(), npc.getZ(), npc.getHeading());
        npc.teleToLocation(loc, player.getReflectionId());
        location();
    }
    ...
    public void rotateR() {
        L2Player player = (L2Player) getSelf();
        L2NpcInstance npc = (L2NpcInstance) player.getTarget();
        Location loc = new Location(npc.getX(), npc.getY(), npc.getZ(), npc.getHeading() + 1000);
        npc.setHeading(npc.getHeading() + 1000);
        final GArray<L2Character> aroundCharacters = npc.getAroundCharacters(1000, 500);
        for (L2Character character : aroundCharacters) {
            character.sendPacket(new NpcInfo(npc, player));
        }
        location();
    }
    ...
    public void saveLoc() {
        ...
        if (player.isGM()) {
            ...
            try {
                file = new FileWriter("saveLocSpawn.txt", true);
                sb.append("new Location( ").append(npc.getX()).append(", ").append(npc.getY()).append(" ,").append(npc.getZ()).append(" ,").append(npc.getHeading()).append("); \n");
    ...
    public void map() {
        ...
        if (player == null || npc == null)
            return;
        StringBuilder dialog = new StringBuilder("<html><body>");
        dialog.append("<table width=\"300\" border=0> ");
        dialog.append("<tr>");
        dialog.append("<td width=80 height=15></td>");
        dialog.append("\t<td><button value=\"Refresh\" action=\"bypass -h scripts_actions.OnActionShift:map\" width=80 height=25 back=\"L2UI_CT1.Button_DF\" fore=\"L2UI_CT1.Button_DF\"></td>");
        dialog.append("</tr>");
        dialog.append("<tr>");
        dialog.append("<td width=80 height=15></td>");
        dialog.append("<td width=80 height=15><img src=\"L2Font-ru.WorldMap6\"\n" +
    ...
    public void location() {
        ...
        StringBuilder dialog = new StringBuilder("<html><body>");
        dialog.append("<table width=\"100%\" border=0>");
        dialog.append("<tr>");
        dialog.append("<td width=80 height=15></td>");
        dialog.append("\t<td><button value=\"W\" action=\"bypass -h scripts_actions.OnActionShift:moveW\" width=80 height=25 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></td>");
        dialog.append("<td width=80 height=15></td>");
        dialog.append("<td width=80 height=15></td>");
        dialog.append("</tr>");
        dialog.append("<tr>");
        dialog.append("\t<td><button value=\"A\" action=\"bypass -h scripts_actions.OnActionShift:moveA\" width=80 height=25 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></td>");
        dialog.append("<td width=80 height=15></td>");
        dialog.append("\t<td><button value=\"D\" action=\"bypass -h scripts_actions.OnActionShift:moveD\" width=80 height=25 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></td>");
        dialog.append("<td width=80 height=15></td>");
        dialog.append("</tr>");
        dialog.append("<tr>");
        dialog.append("<td width=80 height=15></td>");
        dialog.append("\t<td><button value=\"S\" action=\"bypass -h scripts_actions.OnActionShift:moveS\" width=80 height=25 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></td>");
        dialog.append("<td width=80 height=15></td>");
        dialog.append("<td width=80 height=15></td>");
        dialog.append("</tr>");
        dialog.append("<tr>");
        dialog.append("\t<td><button value=\"влево\" action=\"bypass -h scripts_actions.OnActionShift:rotateR\" width=80 height=25 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></td>");
        dialog.append("<td width=80 height=15></td>");
        dialog.append("\t<td><button value=\"вправо\" action=\"bypass -h scripts_actions.OnActionShift:rotateL\" width=80 height=25 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></td>");
        dialog.append("<td width=80 height=15>").append(npc.getHeading()).append("</td>");
        dialog.append("</tr>");
        dialog.append("<tr>");
        dialog.append("<td width=80 height=15></td>");
        dialog.append("\t<td><button value=\"save\" action=\"bypass -h scripts_actions.OnActionShift:saveLoc\" width=80 height=25 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></td>");
        dialog.append("<td width=80 height=15></td>");
        dialog.append("<td width=80 height=15></td>");
        dialog.append("</tr>");
        dialog.append("</table><br>");
    ...
    public void effects() {
        ...
        if (list != null && !list.isEmpty())
            for (L2Skill s : list) {
                ...
                dialog.append("				</td>");
                dialog.append("				<td>");
                dialog.append("					<br><font color=LEVEL>" + s.getName() + "</font>");
                dialog.append("				</td>");
                dialog.append("				<td>");
                dialog.append("					<br><font color=00ff00>[" + Util.formatTime(npc.getEffectList().getEffectBySkillId(s.getId()).getTimeLeft() / 1000L) + "]</font>");
                dialog.append("				</td>");
                dialog.append("			</tr>");
            }
        dialog.append("		</table>");
        ...
        dialog.append(player.isLangRus() ? "Назад" : "Back");
        dialog.append("\" action=\"bypass -h scripts_actions.OnActionShift:stopEffect\" width=100 height=15 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\" /></center></body></html>");
        show(dialog.toString(), player, null);
        L2Character finalNpc = npc;
        ThreadPoolManager.getInstance().schedule(() -> effects2(player, finalNpc), 1000);
//        executeTask("actions.OnActionShift", "effects2", new Object[] {player, npc}, 1000);
    }

    public void effects2(L2Player player, L2Character npc) {
        if (player == null) {
            return;
        } else if (npc == null && schedule != null) {
            npc = (L2Character) player.getTarget();
            if (npc == null || (!player.isGM() && npc.isPlayable() && npc.getPlayer() != player)) {
                return;
            }
        ...
        dialog.append("		<table border=0 cellspacing=4 cellpadding=2 width=295>");
        ConcurrentLinkedQueue<L2Skill> list = npc.getEffectList().getAllSkills(false);
        L2Character finalNpc1 = npc;
        final List<L2Skill> collect = list.stream().sorted(Comparator.comparingInt(o -> (int) finalNpc1.getEffectList().getEffectBySkillId(o.getId()).getTimeLeft())).collect(Collectors.toList());
...
//        executeTask("actions.OnActionShift", "effects2", new Object[] {player, npc}, 1000);
    ...
    public void stopEffect() {
        L2Player player = (L2Player) getSelf();
        player.sendMessage("Остановочка");
        effects2(player, null);
