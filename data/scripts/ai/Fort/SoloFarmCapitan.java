...
public class SoloFarmCapitan extends DefaultAI
{
	...
	private static final int monsterId = 20249;

	public SoloFarmCapitan(L2NpcInstance actor)
	{
		super(actor);
	...
	protected void onEvtThink()
	{
		...
		try
		{
			if(getIntention() == CtrlIntention.AI_INTENTION_ACTIVE || getIntention() == CtrlIntention.AI_INTENTION_IDLE)
				...
				GArray<L2NpcInstance> npcs = actor.getAroundNpc(300, 200);
				L2NpcInstance target = null;
				for (L2NpcInstance npc : npcs) {
					if (npc.getNpcId() == monsterId){
						target = npc;
						actor.getAI().notifyEvent(CtrlEvent.EVT_AGGRESSION, npc, 1);
		...
		if(ref != null && _chatTimer < System.currentTimeMillis())
		{
			...
			switch(ref.getInstancedZoneId())
			{
				case 1003:
					...
					break;
				case 1004:
					QuestState qs2 = masterplayer.getQuestState("_10601_FortSoloFarm");
					...
					QuestState qs3 = masterplayer.getQuestState("_10601_FortSoloFarm");
					...
					QuestState qs4 = masterplayer.getQuestState("_10601_FortSoloFarm");
