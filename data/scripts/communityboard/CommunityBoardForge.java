    ...
    public void parsecmd(String command, L2Player player) {
        ...
        if (command.equals("_bbsforges"))
            ...
            if (head != null) {
                ...
                if (head.isAugmented() || !canBeAugmented(head, player, false)) {
                    HeadButton = immposible;
                    HeadPic = slotclose;
                } else {
                    if (itemCheckGrade(player, head)) {
                        if (head.getItem().isHeroWeapon()) {
                            HeadButton = heronot;
                            HeadPic = slotclose;
                        } else {
                            HeadButton = "<button action=\"bypass -h _bbsforges:augment:item:0:" + Inventory.PAPERDOLL_HEAD + "\" value=\"" + new CustomMessage("common.enchant.augment", player).toString() + "\" width=140 height=25 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\">";
                            if (head.getItem().isPvP())
                                HeadPic = pvp;
                            else
                                HeadPic = picenchant;
                        }
            ...
            if (chest != null) {
                ...
                if (chest.isAugmented() || !canBeAugmented(chest, player, false)) {
                    ChestButton = immposible;
                    ChestPic = slotclose;
                } else {
                    if (itemCheckGrade(player, chest)) {
                        if (chest.getItem().isHeroWeapon()) {
                            ChestButton = heronot;
                            ChestPic = slotclose;
                        } else {
                            ChestButton = "<button action=\"bypass -h _bbsforges:augment:item:0:" + Inventory.PAPERDOLL_CHEST + "\" value=\"" + new CustomMessage("common.enchant.augment", player).toString() + "\" width=140 height=25 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\">";
                            if (chest.getItem().isPvP())
                                ChestPic = pvp;
                            else
                                ChestPic = picenchant;
            ...
            if (legs != null) {
                ...
                if (legs.isAugmented() || !canBeAugmented(legs, player, false)) {
                    LegsButton = immposible;
                    LegsPic = slotclose;
                } else {
                    if (itemCheckGrade(player, legs)) {
                        if (legs.getItem().isHeroWeapon()) {
                            LegsButton = heronot;
                            LegsPic = slotclose;
                        } else {
                            LegsButton = "<button action=\"bypass -h _bbsforges:augment:item:0:" + Inventory.PAPERDOLL_LEGS + "\" value=\"" + new CustomMessage("common.enchant.augment", player).toString() + "\" width=140 height=25 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\">";
                            if (legs.getItem().isPvP())
                                LegsPic = pvp;
                            else
                                LegsPic = picenchant;
                        }
            ...
            if (gloves != null) {
                ...
                if (gloves.isAugmented() || !canBeAugmented(gloves, player, false)) {
                    GlovesButton = immposible;
                    GlovesPic = slotclose;
                } else {
                    if (itemCheckGrade(player, gloves)) {
                        if (gloves.getItem().isHeroWeapon()) {
                            GlovesButton = heronot;
                            GlovesPic = slotclose;
                        } else {
                            GlovesButton = "<button action=\"bypass -h _bbsforges:augment:item:0:" + Inventory.PAPERDOLL_GLOVES + "\" value=\"" + new CustomMessage("common.enchant.augment", player).toString() + "\" width=140 height=25 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\">";
                            if (gloves.getItem().isPvP())
                                GlovesPic = pvp;
                            else
                                GlovesPic = picenchant;
                        }
            ...
            if (feet != null) {
                ...
                if (feet.isAugmented() || !canBeAugmented(feet, player, false)) {
                    FeetButton = immposible;
                    FeetPic = slotclose;
                } else {
                    if (itemCheckGrade(player, feet)) {
                        if (feet.getItem().isHeroWeapon()) {
                            FeetButton = heronot;
                            FeetPic = slotclose;
                        } else {
                            FeetButton = "<button action=\"bypass -h _bbsforges:augment:item:0:" + Inventory.PAPERDOLL_FEET + "\" value=\"" + new CustomMessage("common.enchant.augment", player).toString() + "\" width=140 height=25 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\">";
                            if (feet.getItem().isPvP())
                                FeetPic = pvp;
                            else
                                FeetPic = picenchant;
                        }
            ...
            if (_item.getItem().isWeapon()){
                option_id = ConfigValue.WeaponAugmentOptionList;
            }else if (_item.getItem().isAccessory()){
                option_id = ConfigValue.JewelAugmentOptionList;
            }else if (_item.getItem().isArmor()){
                option_id = new int[]{ 25013, 25020, 25039, 25054, 25061, 25087, 25094, 25108};
            }
            ...
            for (int i = CharactersStart; i < CharactersEnd; i += 3) {
                ...
                html.append("<font color=\"LEVEL\"><a action=\"bypass -h _bbsforges:augment:" +
                        option_id[i] + ":" + item + ":" + (_item.getItem().isWeapon() ? WeaponAugmentOptionItemId[i] : JewelAugmentOptionItemId[i]) +
                        ":" + (_item.getItem().isWeapon() ? WeaponAugmentOptionCount[i] : JewelAugmentOptionCount[i]) + "\">" +
                        (skill == null ? AugmentName.getString(option_id[i]) : skill.getName()) + "</a></font><br1>› <font color=8DB600>Цена:</font> " +
                        (_item.getItem().isWeapon() ? WeaponAugmentOptionCount[i] : JewelAugmentOptionCount[i]) + " " + name + "");
                html.append("</td>");
                html.append("</tr>");
                html.append("</table>");
                html.append("</td>");
...
//
    ...
    public boolean canBeAugmented(L2ItemInstance item, L2Player player, boolean isAccessoryLifeStone) {
        if (!item.canBeEnchanted())
            ...
            return false;
        else if (item.getItem().getItemGrade().ordinal() < Grade.D.ordinal())
            return false;
...
//        if ((isAccessoryLifeStone ? itemType != L2Item.TYPE2_ACCESSORY : itemType != L2Item.TYPE2_WEAPON) && !ConfigValue.AugmentAll)
        if ((isAccessoryLifeStone ? itemType != L2Item.TYPE2_ACCESSORY : false) && !ConfigValue.AugmentAll)
            return false;
