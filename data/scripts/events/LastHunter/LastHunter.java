...
public class LastHunter extends Functions implements ScriptFile {
    private static Logger _log = Logger.getLogger(LastHunter.class.getName());
    ...
    private static Map<L2Player, ArrayList<L2ItemInstance>> items_collector = new HashMap<L2Player, ArrayList<L2ItemInstance>>();
    private static Map<L2Player, ArrayList<L2ItemInstance>> items_antidiup = new HashMap<L2Player, ArrayList<L2ItemInstance>>();
    ...
    private static int[] points = {armorPoint, weaponPoint, buffPoint};
    private static int mob = 36646;

    private static ArrayList<Location> spawnLocations = new ArrayList<>(Arrays.asList(
    ...
    private static ArrayList<Location> spawnPoints = new ArrayList<>(Arrays.asList(
    ...
    private static ArrayList<Location> spawnMobs = new ArrayList<>(Arrays.asList(
    ...
    private static L2Zone _zone = ZoneManager.getInstance().getZoneByIndex(ZoneType.battle_zone, 9003, true);
...
//                    .replaceAll("content", htmlButton(value, action, 200, 32));
    ...
    public void start(String[] var) {
        ...
        sayToAll("scripts.events.LastHunter.AnnouncePreStart", param);
    ...
    public static void question() {
        for (L2Player player : L2ObjectsStorage.getPlayers())
            if (player != null && player.getLevel() >= _minLevel && player.getLevel() <= _maxLevel && player.getReflection().getId() <= 0 && !player.isInOlympiadMode() && !player.isDead() && !player.isInZone(ZoneType.epic) && !player.isFlying() && player.getVar("jailed") == null && player.getVarB("event_invite", true))
                player.scriptRequest(new CustomMessage("scripts.events.LastHunter.AskPlayer", player).toString(), "events.LastHunter.LastHunter:addPlayer", new Object[0]);
    }

    public static void announce() {
        if (players_list.size() < 1) {
            sayToAll("scripts.events.LastHunter.AnnounceEventCancelled", null);
        ...
        if (_time_to_start > 1) {
            ...
            _status = 1;
            _isRegistrationActive = false;
            sayToAll("scripts.events.LastHunter.AnnounceEventStarting", null);
...
//        }
    ...
    public static void prepare() {
        ...
        sayToAll("scripts.events.LastHunter.AnnounceFinalCountdown", null);
    }
    ...
    public static void go() {
        ...
        clearArena();
        spawnPoint();
        sayToAll("scripts.events.LastHunter.AnnounceFight", null);
...
//        final Location location = new Location(23656, -7336, -1216);
//        //Functions.spawn( location, 36647, _eventDuration, reflection.getId());
//        mobs.add(Functions.spawn( location, 36647, _eventDuration, reflection.getId()));
    }
    ...
    public static void endBattle() {
        ...
        if (live_list.size() == 1) {
            for (HardReference<L2Player> ref : live_list) {
                ...
                sayToAll("scripts.events.LastHunter.AnnounceWiner", repl);
        ...
        sayToAll("scripts.events.LastHunter.AnnounceEnd", null);
...
//            }
    ...
    public void buffPlayer(String[] s) {
        ...
        if (player != null) {
            if (!player.getEffectList().containEffectFromSkills(new int[]{1044})) {  //проверка на баф Regeneration
                buff = firstBuffs;
